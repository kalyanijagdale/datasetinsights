...
def create_loader(
    ...
    """Create io loader from dataset

    Note: The io loader here is a pytorch io loader object which does not
    ...
    Args:
        dataset (Dataset): dataset object derived from
            `datasetinsights.io.datasets.Dataset` class.
        ...
        num_workers (int): number of parallel workers used for io loader.
            Set to `0` to run on a single thread (instead of `1` which might
            introduce overhead). (default: 0)
    Returns:
        `torch.utils.data.DataLoader` object as io loader
    """
